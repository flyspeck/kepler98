...
We use the function $\chi$ defined in [1.8]. 
...
\proclaim{Corollary 3.2.2} The quadrilateral regions shown in Fig. 3.1 are the only
...
Formulas for $\dih, \sol, \rad, \eta, \Delta, \vol$  can be found in [1.8].  
...
See [1.8.6] for a discussion of the uses of Rogers simplices in the Kepler conjecture.
...
\proclaim{Lemma 5.3.2} $\vol(V(S_0))=4 \pi M + \mu(V(S_0))$. \endproclaim
...
\proclaim{Theorem 6.3.1} If a vertex has 4 tetrahedra and 1 exceptional region with $n\ge5$ sides, then $\sum^S\mu > \sum^S t_n+0.016$ where $S$ is the set of 4 tetrahedra and the exceptional region.
\endproclaim
