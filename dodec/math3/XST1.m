...
CorrectionTerms[cmin_,cmax_] := 
  Module[{stream, x, clopen, out, vv,r,type,rj,j,i}, 
    ...
    stream = OpenWrite["/var/tmp/Z3/subdiv"]; 
	streamABORT=stream;
	...
	out[tail];
	Close[stream];
    clopen["/var/tmp/Z3/subdiv.log"]; clopen["/var/tmp/Z3/x.log"]; clopen["/var/tmp/Z3/y.log"]; 
    Run["cplex < /var/tmp/Z3/subdiv.exec"]; clopen["/var//tmp/Z3/subdiv.sum"]; 
	Run["sed -f /tmp/getAddValues /tmp/subdiv.log > /tmp/subdiv.sum"];
	Run["cat /tmp/getAddTail >> /var/tmp/Z3/subdiv.sum"];
    << "/var/tmp/Z3/subdiv.sum"; 
...
tauA[x_]:= (Squander @@ x[[{1,2,3,4,5,6}]]);
tauB[x_]:= (Squander @@ x[[{1,3,8,7,9,5}]]);
tauC[x_]:= (Squander @@ x[[{1,8,12,10,11,9}]]);
tauD[x_]:= (Squander @@ x[[{1,12,14,13,15,11}]]);
tauE[x_]:= 0;
...
cfsqp[x_]:= Module[{stream,i},
	stream = OpenWrite["/var/tmp/Z3/subdiv_cfsqp.dat"];
	Do[WriteString[stream,ConvertString[x[[i]]]],{i,1,Length[x]}];
	streamABORT=stream;
	Close[stream];
	Run["/tmp/partU"];
	<< /var/tmp/Z3/subdiv_cfsqp.m;
	cfsqpVal
	];  
