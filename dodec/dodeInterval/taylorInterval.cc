...
static int setVoronoiVolume(const domain& x,const domain& z,double DD[6][6])
	{
	double X[6],Z[6];
	int i;
	for (i=0;i<6;i++) { X[i]=x.getValue(i); Z[i]=z.getValue(i); }
	return secondDerive::setVoronoiVolume(X,Z,DD);
	}
primitive voronoiVolume(linearization::volume,setVoronoiVolume);
const taylorFunction taylorSimplex::voronoiVolume(::voronoiVolume);
...
static int setTruncatedVoronoiVolume(const domain& x,const domain& z,double DD[6][6])
	{
	double X[6],Z[6];
	int i;
	for (i=0;i<6;i++) { X[i]=x.getValue(i); Z[i]=z.getValue(i); }
...
primitive truncatedVoronoiVolume(linearization::truncatedVolume,setTruncatedVoronoiVolume);
const taylorFunction taylorSimplex::truncatedVoronoiVolume(::truncatedVoronoiVolume);
