...
lineInterval linearization::volume(const domain &x)
{
        double x1,x2,x3,x4,x5,x6;
        x1 = x.getValue(0); x2 = x.getValue(1); x3 = x.getValue(2);
        x4 = x.getValue(3); x5 = x.getValue(4); x6 = x.getValue(5);
        lineInterval del = sqrt(linearization::delta(x));
        static const interval f48 = "48.0";
        lineInterval u126 = U126(x1,x2,x6);
        lineInterval u135 = U135(x1,x3,x5);
        lineInterval u234 = U324(x3,x2,x4);
        lineInterval vol =
                f126(x1,x2,x3,x4,x6,x6)*chi126(x1,x2,x3,x4,x5,x6)/u126 \
+
                f324(x1,x2,x3,x4,x5,x6)*chi324(x)/u234 +
                f135(x1,x2,x3,x4,x5,x6)*chi135(x1, x2, x3, x4, x5, x6)/\
u135;
        vol = vol/(del*f48);
        return vol;
}

lineInterval linearization::truncatedVolume
        (const domain &x)
{
          double x1,x2,x3,x4,x5,x6;
        x1 = x.getValue(0); x2 = x.getValue(1); x3 = x.getValue(2);
        x4 = x.getValue(3); x5 = x.getValue(4); x6 = x.getValue(5);
          lineInterval V;
          interval cut("1.2584085723648189697057567");
          V=vorVc(x1,x2,x3,x4,x5,x6,cut);
          lineInterval volTrunc;
          static const interval three("3");
          static const interval f_3 ("1.333333333333333333333333333333"\
);

          static const interval m4doct ="-2.88361179806985958";
          volTrunc=(V-f_3*solid(x))/m4doct;
...
int edgeBound::x4_upper_from_dih_upper
	...
	if (((x1<6.3344)&&(x4<8))
		...
		(x2>6.3344)||(x3>6.31)||(x5>6.31)||(x6>6.3344))
		{
...
void linearization::selfTest() {
   ...
   -0.05217343132181529);
	...
	double t=12.6;
	if (!edgeBound::shortDiagMax(4.1,4.2,4.3,4.4,6.3344,t,4.5,4.6,4.7,4.8))
	...
	t=12.6;
	if (!edgeBound::shortDiagMax(5.1,5.2,4.3,4.4,6.3344,t,4.5,4.6,4.7,4.8))
		cout << "shortDiagMax failed..." << endl;
