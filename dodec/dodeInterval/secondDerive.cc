...
// deltaX=  -(x1*x2) - x0*x3 + x1*x4 + x2*x5 - x4*x5 + x0*(-x0 + x1 + x2 - x3 + x4 + x5);
void setDeltaX(const double x[6],const double z[6],
	interval& f, interval Df[6], interval DDf[6][6])
	{
...
static int dihfactor(double x,double z,interval t,interval& r,interval& Dr,
		interval& DDr)
	{
...
int dihVc(double x,double z,interval trunc,
	...
	{
	interval r,Dr,DDr;
	if (!(dihfactor(x,z,trunc,r,Dr,DDr))) return 0;
	int i,j;
	for (i=0;i<6;i++) for (j=i;j<6;j++)
		DDc[i][j] = r*DDd[i][j];
	for (i=0;i<6;i++) for (j=0;j<i;j++)
...
int dih2Vc(double x,double z,interval trunc,
	const interval& d,const interval Dd[6],const interval DDd[6][6],
	interval DDc[6][6])
...
int dih3Vc(double x,double z,interval trunc,
	const interval& d,const interval Dd[6],const interval DDd[6][6],
	interval DDc[6][6])
...
int secondDerive::setVorSqc
	...
	static const interval three("3");
	static const interval four("4");
	static const interval t("1.2584085723648189697");
	...
	static const interval doct3=three*doct;
	static const interval tdoct4quoin= 
		two*interval::wideInterval("-0.005","0.005")*doct4; 
		 // nondiag max abs = 0.10629. 
		 // from out.quoin.simplex.sqrt.
		 // factor of 2 because 2 quoins on
		 //   each face
	static const interval s63("6.33436854000504729");
	...
	if (!dihVc(x[0],z[0],t, f,Df,DDf1,DDx1)) return 0;
	if (!setDih2(x,z,s,Ds,DDs,f,Df,DDf2)) return 0;
	if (!dih2Vc(x[1],z[1],t, f,Df,DDf2,DDx2)) return 0;
	if (!setDih3(x,z,s,Ds,DDs,f,Df,DDf3)) return 0;
	if (!dih3Vc(x[2],z[2],t, f,Df,DDf3,DDx3)) return 0;
	for (i=0;i<6;i++) for (j=i;j<6;j++)
		{
	...
	static const interval diag = 
		two*interval::wideInterval("-0.009","0.009")*doct4;
				// diag max abs = 0.147748...
				// from out.quoin.simplex.sqrt;
	if (x[5]<=6.33436854000504729)
		{
	...
	//convert to truncated volume
	
	interval sqrt_d,Dsqrt_d[6],DDsqrt_d[6][6];
	interval DDy[6][6];

	if (!setSqrtDelta(x,z,sqrt_d,Dsqrt_d,DDsqrt_d)) return 0;
	if (!secondDerive::setSolid(x,z,sqrt_d,Dsqrt_d,DDsqrt_d,DDy)) return 0;

	interMath::up();
	for (i=0;i<6;i++) for (j=0;j<6;j++)
	     {
	DDv[i][j]=DDv[i][j]/interMath::inf(doct4)+interMath::sup( interMath::max(DDy[i][j],-DDy[i][j]))/interMath::inf(doct3);
...
int secondDerive::setVoronoiVolume(const double x[6],const double z[6],
    double DD[6][6])
	{
    ...
    interval DDx[6][6];
    if (!secondDerive::setSqrtDelta(x,z,sqrtd,Dsqrtd,DDsqrtd)) return 0;
    if (!voronoiVolume(x,z,sqrtd,Dsqrtd,DDsqrtd,DDx)) return 0;
    interMath::up();
    for (i=0;i<6;i++) for (j=i;j<6;j++)
...
static void quasiregular_partition() // Used to generate 2nd d. bounds.
	{
	static const interval zero("0");
	static const interval two("2");
	static const interval three("3");
	...
	for (k[5]=0;k[5]<top;k[5]++)
		...
		dihVc(x[0],z[0],trunc0,di,Ddi,DDdi,DDVc);
		dihVc(x[0],z[0],trunc1,di,Ddi,DDdi,DDc);
...
static void dih2_partition() // used to generate 2nd derivative bounds
	{
	static const interval zero("0");
	static const interval two("2");
	static const interval four("4");
	...
	for (k[5]=0;k[5]<top;k[5]++)
		...
		dih2Vc(x[1],z[1],trunc0,di,Ddi,DDdi,DDVc);
		dih2Vc(x[1],z[1],trunc1,di,Ddi,DDdi,DDc);
		for (i=0;i<6;i++) for (j=0;j<6;j++)
			{
			fDDdi[i][j]=interMath::combine(fDDdi[i][j],DDdi[i][j]);
			fDDVc[i][j]=interMath::combine(fDDVc[i][j],DDVc[i][j]);
...
void secondDerive::selfTest() {
	static const interval zero("0");
    cout << " -- loading derivative routines $Revision: 1.3 $ \n" << flush;
   ...
   {0.002626619500685369, 0.002627318650262751, 0.01184563865055325, 
	...
	/*test dihfactor*/{
	interval r,Dr,DDr;
	dihfactor(4.1,4.1,interval("1.255"),r,Dr,DDr);
   ...
   {0.005065416998339743, 0.0014398974979986, -0.001196669350828044, 
	...
	secondDerive::setSqrtDelta(x,x,s,Ds,DDs);
	secondDerive::setDihedral(x,x,s,Ds,DDs,f,Df,DDf); 
	dihVc(x[0],x[0],interval("1.255"), f,Df,DDf,DDx1); 
   ...
   {0.0004158696512009301, 0.0003321760267211582, 0.008091450771502849, 
	...
	cout << " -- untested procedures  "
		<< " setVorVcInverted, setVorSqc, ";
	cout << "array_multiply " << endl << flush;
