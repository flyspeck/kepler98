...
void verify_cell( double tet[4], struct Cell_Info info );
...
void alpha_fun( double y[4], double out[2] );
void sol_fun( double y[4], double out[2] );
void cos_arc_fun( double y[6], double out[2] );
void solc_fun( double y[2], double al[2], double out[2] );
void vor0_fun( double y[2], double al[2], double out[2] );
void tau0_fun( double y[2], double al[2], double out[2] );
void f_fun( double y[4], double out[2] );
...
void verifypoly( void )
{
	...
	printf("Welcome to Section A_14.9.\n");
...
void verify_cell( double y[4], struct Cell_Info info )
{
	int i, j, n0, n1, bounds[2];
	double val;
	double delta[2];
	double yp[4], t[4], diff[2];
...
void alpha_fun( double y[4], double out[2] )
{
	...
	bigy[8] = ell_val[0];
	bigy[9] = ell_val[1];
	bigy[10] = lambda_val[0];
	bigy[11] = lambda_val[1];
...
{
	int i;
	double x[12], bigy[12], delta[2], sqrtdelta[2];
...
void vor0_fun( double y[2], double al[2], double out[2] )
{
	double h[2], p1[2], p2[2], solc[2], aval[2];
	
	h[0] = 0.5*y[0];
	h[1] = 0.5*y[1];
	solc_fun( y, al, solc );
	a_fun( h, aval );
	i_mult( solc, phi0_val, p1 );
	i_mult( aval, al, p2 );
	i_add( p1, p2, out );
}
...
void tau0_fun( double y[2], double al[2], double out[2] )
{
	double solc[2], vor0_val[2], temp[2];
	
	solc_fun( y, al, solc );
	vor0_fun( y, al, vor0_val );
	i_mult( solc, zetapt_val, temp );
	i_sub( temp, vor0_val, out );
}


void f_fun( double y[4], double out[2] )
{
	double ty[4], temp[2], al1[2], al2[2];
	double al1A[2],  al2A[2], sol_val[2];
	double tau1_val[2], tau2_val[2];
	double Ay1[2], Ay2[2];
	double p1[2];
	
	sol_fun( y, sol_val );
	i_sub( zetapt_val, phi0_val, temp );
	i_mult( temp, sol_val, p1 );
	...
	ROUND_DOWN;
	temp[0] = PI_LO - 2.0*al1[1];
	ROUND_UP;
	temp[1] = PI_HI - 2.0*al1[0];
	
	tau0_fun( y, temp, tau1_val );

	ROUND_DOWN;
	temp[0] = PI_LO - 2.0*al2[1];
	ROUND_UP;
	temp[1] = PI_HI - 2.0*al2[0];
	
	tau0_fun( y+2, temp, tau2_val );
	
	ROUND_DOWN;
	out[0] = 2.0*(p1[0] - al1A[1] - al2A[1]) + tau1_val[0] +
		tau2_val[0];
	ROUND_UP;
	out[1] = 2.0*(p1[1] - al1A[0] - al2A[0]) + tau1_val[1] +
		tau2_val[1];
}
...
2.1345 2.0987 2.1345 2.0987 2.1345 2.0987
*/
