...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.9.2.  \n");
...
void set_relation( int num )
{
	int i, j;
	double bigmat[1][7] = 
		{{-0.492, -0.492, -0.492, 0.43, 0.038, 0.038, -2.71884}};
	...
	double out[2], double out_partials[12] )
{
	...
	*/
	...
	i_solid( y, sqrtdelta, rel_val );
	s_solid_partials( y, delta_val, sqrtdelta, delta_part, t );
	...
	*/
	...
	for( i=0; i<6; i++ ) {
		j = 2*i;
		jp = j + 1;
		out_partials[j] = y_coefficients[j] - t[jp];
	}
	ROUND_UP;
	for( i=0; i<6; i++ ) {
		j = 2*i;
		jp = j + 1;
		out_partials[jp] = y_coefficients[jp] - t[j];
	}
