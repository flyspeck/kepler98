...
void verifypoly( void )
{
	...
	printf("Welcome to Section A_14.6.\n");
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	double val;
	double x[12], xp[12], delta_part[12], delta[2];
	double yp[12], t[12], data[2], diff[6], xface[6];
	double crad32[2];
	...
	*/
	...
	/* i_crad3x2( xface, crad32 ); */
	s_crad3x2( xface, crad32 );
	if( crad32[0] > t0_val2[1] ) {
		dualcount += 1.0;
		return;
	}
	...
	/* if( delta[0] >= 421.0 ) { */
	if( delta[0] > slop && crad32[1] < t0_val2[0] ) {
		...
		printf("t0^2-crad3^2 = [%0.18g\t%0.18g]\n", 
			t0_val2[0] - crad32[1], t0_val2[1] - crad32[0]);
		return;
	}
	...
	/* only use full partials information for y2, y4, y6 */
	for( j=1; j<6; j+=2 ) {
		i = 2*j;
		n0 = i + 1;
		if( diff[j] > 0.0 ) {	/* ignore tight spots */
			if( delta_part[n0] < 0.0 ) {
	...
	/* only shrink for y1, y3, y5, so as to avoid crad3 constraint */
	for( j=0; j<6; j+=2 ) {
		i = 2*j;
		n0 = i + 1;
		if( diff[j] > 0.0 ) {	/* ignore tight spots */
