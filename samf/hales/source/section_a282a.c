...
void minustau_gma( double y[12], double out[2] );
...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.2.8.2a.  \n");
	...
	for( n=1; n < 3; n++ ) { 	
		...
		switch( n ) {
			case 1:		
				...
				i_mult( tetdiff, tetdiff + 2, tetdiff + 4 );
				
				dt = -0.06585;
				i_recognize( dt, tetdiff + 2 );
				i_add( tetdiff + 2, tetdiff + 4, tetdiff );
				
				tet[2] = two212[0];
				tet[3] = two212[1];
				tet[6] = sean_val[0];
				tet[7] = sean_val[1];
				minustau_gma( tet, tetdiff + 2 );
				i_sub( tetdiff, tetdiff + 2, scorebd );
