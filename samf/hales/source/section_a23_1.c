...
*/	
	...
	printf("Welcome to Section A_23.1.  \n");
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	if( data2[1] < sean_val2[0] ) {	/* discard */
		vorcount += 1.0;
		return;
	}
...
*/
	...
	data[0] = vor0_val[0] + vor0p_val[0] + 
							relation_const[2]*dih_val[1];
	ROUND_UP;
	data[1] = vor0_val[1] + vor0p_val[1] + 
							relation_const[3]*dih_val[0];
...
#if PARTIALS
	/* don't need to check data2[0] > sean_val2[1] since this
	boundary case is included in the next set of verifications. */
	if( info.depth > PARTIALSTART )	{
/*
	if( dih_val[1] < two46[0] &&
			info.depth > PARTIALSTART )	{
*/
...
void set_relation( int num )
{
	...
	i_recognize( relations[num][0] - 0.06585, relation_const );
