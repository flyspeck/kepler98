...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.12.4b.  \n");
	/* Mis-labeled originally. */
	/* This is the boundary case where yp4==2.51. */
	...
	for( n=1; n < 3; n++ ) { 
		...
		if( n==1 ) {
			i_recognize( -0.136, tetdiff );
		} else {
			i_recognize( -0.224, tetdiff );
		...
		*/
		...
		for( subcase=casestart; subcase<=casebound; subcase++ ) {
			...
			/* for( subsub=0; subsub<3; subsub++ ) { */
			for( subsub=-1; subsub<0; subsub++ ) {
							
				for( i=0; i<12; i+=2 ) {
					tet[i  ] = 2.0;
					tet[i+1] = sean_val[1];
				}
				i_recognize( 2.696, tetdiff );
				tet[0] = tetdiff[0];
				tet[1] = TWOSQRT2_HI;
				...
				/* If we use dimension-reduction, we have to
				check the boundary, when yp4==2.51. */
				if( subsub==0 ) {
					printf("\t\ty1 = 2.696\n");
					tet[1] = tet[0];
				}
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	double x[12], xp[12], yp[12], ynew[12];
	...
	if( data2[1] < sean_val2[0] || 
			data2[0] > sean_val2[1] ) {	/* discard */
		vorcount += 1.0;
		return;
	}
	...
	if( data[0] > slop && data2[0] > sean_val2[1] ) {
/*
	if( data[0] > slop && data2[0] > sean_val2[1] && 
			dih_val[1] < two46[0] ) {
*/
