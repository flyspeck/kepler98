	...
	for( n=1; n < 5; n++ ) { 	
		...
		switch( n ) {
			case 1:		
				...
				scorebd[0] = zero114[0]/4.0;
				scorebd[1] = zero114[1]/4.0;
				ROUND_NEAR;
			...
			case 2:
				...
				tetdiff[2] = 2.0*tetdiff[0];
				tetdiff[3] = 2.0*tetdiff[1];
				i_sub( zero114, tetdiff + 2, tetdiff );
				scorebd[0] = 0.5*tetdiff[0];
				scorebd[1] = 0.5*tetdiff[0];
				ROUND_NEAR;
				printf("scorebd = [%.18f, %.18f]\n", 
					scorebd[0], scorebd[1]);
				for( i=0; i<12; i+=2 ) {
			...
			case 3:
				...
				i_sub( zero114, tetdiff, tetdiff + 2 );
				ROUND_DOWN;
				scorebd[0] = tetdiff[2]/3.0;
				scorebd[1] = tetdiff[3]/3.0;
				ROUND_NEAR;
				printf("scorebd = [%.18f, %.18f]\n", 
					scorebd[0], scorebd[1]);
				for( i=0; i<12; i+=2 ) {
