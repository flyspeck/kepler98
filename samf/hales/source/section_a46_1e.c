...
void verifyocta( void )
{
	...
	double diff, dt, origslop, three2[2], three78[2];
	time_t tstart, tstop;
	...
	dt = 3.78;
	i_recognize( dt, three78 );
	i_recognize( dt*dt, three782 );
	
	onefailed = 0;
	
	printf("Welcome to Section A.4.6.1.e.  \n");
	...
	for( n=1; n < 3; n++ ) { 
		...
		if( n==1 ) {
			i_recognize( -0.212, tetdiff );
		} else {
			i_recognize( -0.54525, tetdiff );
				...
				for( a_case[1]=0; a_case[1]<2; a_case[1]++ ) {
					...
					for( a_case[2]=0; a_case[2]<2; a_case[2]++ ) {
						...
						for( a_case[3]=0; a_case[3]<2; a_case[3]++ ) {
							...
							for( a_case[4]=0; a_case[4]<2; a_case[4]++ ) {
								if( a_case[4]==0 ) {
									a_vect[8] = 2.0;
									a_vect[9] = 2.0;
								} else {
								...
								for( a_case[5]=a_case[1]; a_case[5]<2; a_case[5]++ ) {
									if( a_case[5]==0 ) {
										a_vect[10] = 2.0;
										a_vect[11] = 2.0;
									} else {
...
void verify_cell( double y[6], struct Cell_Info info )
{
	...
	double vor01_val[2], vor02_val[2], vor03_val[2], vor04_val[2];
	double delta1[2], delta2[2], delta3[2], delta4[2];
	double sqrtdelta[2];
	...
	if( data2[1] < three22[0] || 
			data2[0] > three782[1] ) {	/* discard */
		sccutcount += 1.0;
		return;
	}
	...
	i_vor0( yp, xp, sqrtdelta, vor01_val );
	...
	ROUND_DOWN;
	data[0] = vor01_val[0] + vor02_val[0] + vor03_val[0] + vor04_val[0];
	ROUND_UP;
	data[1] = vor01_val[1] + vor02_val[1] + vor03_val[1] + vor04_val[1];
