...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.6.2.f.  \n");
	...
	for( n=1; n < 3; n++ ) { 
		...
		if( n==1 ) {
			i_recognize( -0.221, tetdiff );
		} else {
			i_recognize( -0.486, tetdiff );
		...
		*/
		for( i=0; i<12; i++ )
			tet[i] = 4.0;
		i_mult( three2, three2, tetdiff );
		tet[6] = tetdiff[0];
		tet[7] = tetdiff[1];
		i_dih( tet, dih_cut );
		ROUND_NEAR;
		printf("\t\tdih_cut = [%.18f, %.18f]\n\n", 
			dih_cut[0], dih_cut[1]);
		...
		for( subcase=casestart; subcase<=casebound; subcase++ ) {
			...
			for( a_case[0]=0; a_case[0]<2; a_case[0]++ ) {
				if( a_case[0]==0 ) {
					a_vect[2] = 2.0;
					a_vect[3] = 2.0;
				} else {
				...
				for( a_case[4]=a_case[0]; a_case[4]<2; a_case[4]++ ) {
					if( a_case[4]==0 ) {
						a_vect[8] = 2.0;
						a_vect[9] = 2.0;
					} else {
...
{
	int i, j, bounds[3], n0, n1;
	double diff[6], t[12], val;
	double xp[12], yp[12], ynew[6];
	double data[2], data2[2];
	double vor02_val[2], vor03_val[2], vor04_val[2];
	double delta2[2], delta3[2], delta4[2];
	double dih2_val[2], dih3_val[2], dih4_val[2];
	double sqrtdelta[2];
	...
	i_bigdelta_best( xp, delta3 );
	
	if( delta3[1] < 0.0 ) {	/* discard */
		vorcount += 1.0;
		return;
	}
	
	i_dih_alt( xp, delta3, dih3_val );
	i_sqrt( delta3, sqrtdelta );
	i_vor0( yp, xp, sqrtdelta, vor03_val );
	...
	i_bigdelta_best( xp, delta4 );
	
	if( delta4[1] < 0.0 ) {	/* discard */
		vorcount += 1.0;
		return;
	}
	
	i_dih_alt( xp, delta4, dih4_val );
	i_sqrt( delta4, sqrtdelta );
	i_vor0( yp, xp, sqrtdelta, vor04_val );
		
	ROUND_DOWN;
	data[0] = vor02_val[0] + vor03_val[0] + vor04_val[0];
	data2[0] = dih2_val[0] + dih3_val[0] + dih4_val[0];
	ROUND_UP;
	data[1] = vor02_val[1] + vor03_val[1] + vor04_val[1];
	data2[1] = dih2_val[1] + dih3_val[1] + dih4_val[1];
	...
	if( data2[1] < dih_cut[0] ) {
		sccutcount += 1.0;
		return;
	}
		
	if( data[0] > slop && data2[0] > dih_cut[1] ) {
		...
		printf("data        = [%0.18g\t%0.18g]\n", 
			data[0], data[1]);
		printf("dih_sum     = [%0.18g\t%0.18g]\n", 
			data2[0], data2[1]);
		printf("vor02_val    = [%0.18g\t%0.18g]\n", 
			vor02_val[0], vor02_val[1]);
		printf("vor03_val    = [%0.18g\t%0.18g]\n", 
			vor03_val[0], vor03_val[1]);
		printf("vor04_val    = [%0.18g\t%0.18g]\n", 
			vor04_val[0], vor04_val[1]);
		printf("delta2       = [%0.18g\t%0.18g]\n", 
			delta2[0], delta2[1]);
		printf("delta3       = [%0.18g\t%0.18g]\n", 
			delta3[0], delta3[1]);
		printf("delta4       = [%0.18g\t%0.18g]\n", 
			delta4[0], delta4[1]);
		return;
	}
