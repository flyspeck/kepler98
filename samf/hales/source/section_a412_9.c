...
void verify_cell( double tet[8], struct Cell_Info info );
void testfuns( void );
void sean_init( void );
...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.12.9.  \n");
...
void verify_cell( double y[8], struct Cell_Info info )
{
	int i, j, bounds[4], n0, n1, n2, n3;
	double diff[6], t[12], val;
	double x[12], xp[12], yp[12], ynew[8];
	double data[2], data2[2];
	double vor01_val[2], vor02_val[2];
	double delta1[2], delta2[2];
	double sqrtdelta[2];
#if PARTIALS
	double partials[12], tpars[12], rel_part[12];
#endif
...
#if PARTIALS
	/* do boundary case separately? */
	if( info.depth > PARTIALSTART && data2[0] > 8.0 )	{
		i_vor0_partials( t, x, tpars );
		i_vor0_partials( yp, xp, partials );
		
		ROUND_DOWN;
		rel_part[0] = tpars[2] + partials[2];
		rel_part[2] = partials[0];
		rel_part[4] = partials[10];
		rel_part[6] = tpars[6] + partials[6];
		ROUND_UP;
		rel_part[1] = tpars[3] + partials[3];
		rel_part[3] = partials[1];
		rel_part[5] = partials[11];
		rel_part[7] = tpars[7] + partials[7];
		
		ROUND_NEAR;
		j = 0;
		for( i=0; i<4; i++ ) {	/* differentials */
