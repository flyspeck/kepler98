...
void verifyocta( void )
{
	...
	printf("Welcome to Section A_19.1.  \n");
		...
		for( subcase=casestart; subcase<=casebound; subcase++ ) {
			...
			switch( n ) {
				...
				case 2:
					...
					break;
				default:
					printf("Somehow reached default case.\n");
					break;
			}
...
{
	int i, j, bounds[6], n0, n1, n2, n3, n4, n5, away;
	...
	if( data2[1] < 8.0 || 
			data2[0] > three22[1] ) {	/* discard */
		vorcount += 1.0;
		return;
	}
	if( do_boundary ) {
		away = (data2[0] > 8.0 && data2[1] < three22[0]);
	} else {
		away = (data2[1] < three22[0]);
	}
...
#if PARTIALS
	if( away && info.depth > PARTIALSTART )	{
