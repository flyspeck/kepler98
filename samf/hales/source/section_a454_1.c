...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.5.4.1.  \n");
...
void set_relation( int num )
{
	int i, j;
	double bigmat[4][8] = 
		{{-0.49, 0.44, 0.44, -0.82, 0.44, 0.44, 
				2.0421, 1.0},
		 {0.495, -0.214, -0.214, 1.05, -0.214, -0.214, 
		 		0.23545, -1.0},
		 {-0.15, 0.38, 0.09, -0.57, 0.54, 0.24, 
		 		2.3398, 1.0},
		 {0.33, -0.375, 0.11, 0.72, -0.36, 0.034, 
		 		-0.36135, -1.0}};
	...
	double out[2], double out_partials[12] )
{
	...
	*/
	...
	ROUND_DOWN;
	for( j=0; j<12; j+=2 ) {
		out_partials[j] = 2.0*compvals[j] + y_coefficients[j];
	}
	ROUND_UP;
	for( j=1; j<12; j+=2 ) {
		out_partials[j] = 2.0*compvals[j] + y_coefficients[j];
