...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.2.7.9.  \n");
...
#if PARTIALS
	if( !do_bdry && (do_tau || dih_val[0] > one32[1]) )	{
...
{
/*
List(0.01228988761700344,-0.002727360626740837,
   -0.005007594392187342,-0.005007594392187342,
   0.005007594392187342,0.002727360626740837,
   0.005007594392187342,0.005007594392187342,
   -0.005007594392187342,-0.01228988761700344,
   0.04727678389524725,-0.04727678389524725,
   -0.07621046552493249,0.07621046552493249,
   -0.05683931088550576,0.05683931088550576,
   -0.04727678389524725,0.04727678389524725,
   0.07621046552493249,-0.07621046552493249,
   0.1045634964896351,0.5310908201333739,
   0.4044115153428878,0.251441384032262,
   0.668290748418542,0.2815236206827469,
   0.1721264416821509)

room = 0.0095119566765638
...
*/
	int i, j;
	double avars[4][5] = 
		{{0.01228988761700344,-0.002727360626740837,
   -0.005007594392187342,-0.005007594392187342,
   0.005007594392187342},{0.002727360626740837,
   0.005007594392187342,0.005007594392187342,
   -0.005007594392187342,-0.01228988761700344},
   ...
   0.668290748418542};
	...
	bval = 0.2815236206827469;
	lambda_val = 0.1721264416821509;
	...
	double out[2], double out_partials[12] )
{
	...
	if( do_tau ) {
		...
		/* Doesn't really matter what we do here, can discard
		anyway.  Simpler to set to 1.32, though. */
		if( compvals[1] < one32[0] ) {
			compvals[1] = one32[0];
			compvals[0] = one32[0];
		} else if( compvals[0] < one32[0] ) {
			compvals[0] = one32[0];
		}
		dih_val[0] = compvals[0];
		dih_val[1] = compvals[1];
	...
	*/
	...
	if( do_quarter && do_tau ) {
		s_crad3x2( x + 2, compvals + 6 );
		ROUND_DOWN;
		out[0] += lambda_val*(compvals[6] - 2.0);
		ROUND_UP;
		out[1] += lambda_val*(compvals[7] - 2.0);
		s_crad3len2_xpars( x + 2, eta2pars );
		ROUND_DOWN;
		for( i=0; i<6; i+=2 ) {
			j = i + 2;
