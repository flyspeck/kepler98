...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.9.1.  \n");
...
void set_relation( int num )
{
	int i, j;
	double bigmat[15][7] = 
		{{0.372, -0.465, -0.465,   0.0, -0.465, -0.465, -4.885},
		 {0.291, -0.393, -0.586, 	0.79, -0.321, -0.397, -2.47277},
		 {0.291, -0.393, -0.586,   0.0, -0.321, -0.397, -4.45567},
		 {0.291, -0.393, -0.586,   0.0, -0.321, -0.397, -4.71107},
		 {0.214,   -0.4,  -0.58,   0.0, -0.155, -0.395, -4.52345},
		 {0.115, -0.452, -0.452, 0.613, -0.15, -0.15, -2.177},
		 {0.115, -0.452, -0.452, 0.618, -0.15, -0.15, -2.17382},
		 {0.115, -0.452, -0.452,   0.0, -0.15, -0.15, -3.725},
		 {0.115, -0.452, -0.452,   0.0, -0.15, -0.15, -3.927},
		 {0.47,  -0.522, -0.522, 0.812, -0.522, -0.522, -2.82998},
		 {0.47,  -0.522, -0.522,   0.0, -0.522, -0.522, -4.8681},
		 {0.47,  -0.522, -0.522,   0.0, -0.522, -0.522, -5.1623},
		 {0.15,   -0.09,   -0.4,   0.0,  -0.23, -0.631, -3.9788},
		 {0.289, -0.148,  -1.36, 0.688, -0.148,  -1.36, -6.3282},
		 {0.289, -0.148, -0.723,   0.0, -0.148, -0.723, -4.85746}};
	...
	double out[2], double out_partials[12] )
{
	...
	*/
	...
	for( i=0; i<6; i++ ) {
		j = 2*i;
		jp = j + 1;
		out_partials[j] = -2.0*compvals[jp] + y_coefficients[j];
	}
	ROUND_UP;
	for( i=0; i<6; i++ ) {
		j = 2*i;
		jp = j + 1;
		out_partials[jp] = -2.0*compvals[j] + y_coefficients[jp];
	}
	...
	for( i=0; i<12; i+=2 ) {
		printf("[%.18f, %.18f]\n", 
			out_partials[i], out_partials[i+1]);
	}
	*/
}
