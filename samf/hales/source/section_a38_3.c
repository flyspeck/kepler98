...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.3.8.3.  \n");
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	double rel_part[12], delta_part[12], partials[12];
	double dih_val[2], special_delta[2];
	...
	t[7] = 8.0;
	ROUND_DOWN;
	t[8] = sean_val[0]*sean_val[0];
	ROUND_UP;
	t[9] = sean_val[1]*sean_val[1];
	
	i_bigdelta_best( t, special_delta );

	if( special_delta[1] < 0.0 ) {
	...
	if( data[0] > slop && dih_val[0] > y_coefficients[1] ) {
		...
		printf("delta       = [%0.18g\t%0.18g]\n", 
			delta[0], delta[1]);
		printf("special_delta = [%0.18g\t%0.18g]\n", 
			special_delta[0], special_delta[1]);
...
#if PARTIALS
	if( special_delta[0] > 0.0 
			&& dih_val[0] > y_coefficients[1] )	{
