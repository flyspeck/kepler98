...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.2.8.1b.  \n");
	...
	for( n=1; n < 5; n++ ) { 	
		...
		for( subcase=casestart; subcase<=casebound; subcase++ ) {
			...
			for( subsub=1; subsub<subsubbound; subsub++ ) {
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	if( data[0] > slop && crad2_val[0] > 2.0 ) {
...
void testfuns( void )
{
	...
	double compvals[14], rel_part[12], data[2];
...
*/
		...
		*/
		...
		fat_composite( taylor_rel, taylor_relconst, y, x, 
			data, rel_part, compvals );
		ROUND_NEAR;
		printf("minustauvor = [%.18f, %.18f]\n", data[0], data[1]);
		do_gma = 1;
		set_relation( 1 );
		fat_composite( taylor_rel, taylor_relconst, y, x, 
			data, rel_part, compvals );
		ROUND_NEAR;
		printf("minustaugma = [%.18f, %.18f]\n", data[0], data[1]);

		do_gma = 0;
		set_relation( 1 );
		taylor_relconst[0] = 0.0;
		taylor_relconst[1] = 0.0;
		fat_composite( taylor_rel, taylor_relconst, y, x, 
			data, rel_part, compvals );
		ROUND_NEAR;
		printf("vor = [%.18f, %.18f]\n", data[0], data[1]);
...
(0, 0)
fred = 
List(0.03333078469988826,0.,0.,0.,0.02382973937167709,0.,
   0.,0.,0.02382973937167709,0.03333078469988826,0.,0.,
   0.,0.0867848604637795,0.1066192891745456,
   0.1066192891745456,0.,0.,0.,0.0867848604637795,
   -0.0153386255778638,0.2645615221710049,
   0.2645615221710049,0.043332322500091,
   0.877937087382042,0.2149230254418398)

room = 0.000318432902566187525

(1, 0)
List(0.07722874794934342,0.,0.,0.,0.05097580656629219,0.,
   0.,0.,0.05097580656629219,0.07722874794934342,0.,0.,
   0.,0.03002467034045119,0.05853561403299467,
   0.05853561403299467,0.,0.,0.,0.03002467034045119,
   -0.1063411991588212,0.1463228207469426,
   0.1463228207469426,-0.01567813306654353,
   0.6610989165213241,0.1188689234475672)

room = 0.000357856800227304727

(0, 1)
List(0.05805081158419911,0.,0.,0.,0.04576450688239486,0.,
   0.,0.,0.04576450688239486,0.05805081158419911,0.,0.,
   0.,0.0565900602120104,0.1309358864164311,
   0.1309358864164311,0.,0.,0.,0.0565900602120104,
   -0.2239113119904678,0.1540620840107873,
   0.1540620840107873,-0.03169629907851767,
   0.822226188825055,0.1251561033129808)

room = 0.00106065756699599278
...
-1.5 pt = -0.0830604685026962208
*/
	int i, j, k;
	double all_avars[3][4][5] = 
		{{{0.03333078469988826,0.,0.,0.,0.02382973937167709},
		{0.,0.,0.,0.02382973937167709,0.03333078469988826},
		{0.,0.,0.,0.0867848604637795,0.1066192891745456},
		{0.1066192891745456,0.,0.,0.,0.0867848604637795}},
		
		{{0.07722874794934342,0.,0.,0.,0.05097580656629219},
		{0.,0.,0.,0.05097580656629219,0.07722874794934342},
		{0.,0.,0.,0.03002467034045119,0.05853561403299467},
   	{0.05853561403299467,0.,0.,0.,0.03002467034045119}},
   	
		{{0.05805081158419911,0.,0.,0.,0.04576450688239486},
		{0.,0.,0.,0.04576450688239486,0.05805081158419911},
		{0.,0.,0.,0.0565900602120104,0.1309358864164311},
  	{0.1309358864164311,0.,0.,0.,0.0565900602120104}}};
	double all_cvals[3][5] = 
		{{-0.0153386255778638,0.2645615221710049,
	   0.2645615221710049,0.043332322500091,
	   0.877937087382042},
	   {-0.1063411991588212,0.1463228207469426,
	   ...
	   0.822226188825055}};
	double all_bvals[3] =
		{0.2149230254418398, 0.1188689234475672, 
			0.1251561033129808};
	double all_room[3] = 
		{0.000318432902566187525, 0.000357856800227304727, 
			0.00106065756699599278};
	double bval, room, avals[6];
	double tet[12], tetdiff[6], dt, scorebd[2];
	...
	avals[0] = 0.0;
	/* y2 coeff */
	avals[1] = all_avars[k][0][i];
	/* y3 coeff */
	avals[2] = all_avars[k][1][i];
	/* y4 coeff */
	avals[3] = 0.0;
	/* y5 coeff */
	avals[4] = all_avars[k][2][i];
	/* y6 coeff */
	avals[5] = all_avars[k][3][i];
	...
	ROUND_DOWN;
	/* slop = origslop - all_cvals[k][i] + room; */
