...
void verifyocta( void )
{
	...
	printf("Welcome to Section A12.2.  \n");
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	double x[12], ynew[12];
	double data[2], data2[2], partials[12];
	double rel_part[12], vor0_val[2], dih_val[2];
	double facex[6], delta[2], sqrtdelta[2], delta_part[12];
	...
	if( do_bdry && data2[0] > 2.0 ) { /* discard */
		dualcount += 1.0;
		return;
	}
	if( data2[1] < 2.0 ) {	/* discard */
		sccutcount += 1.0;
		return;
	}
	...
	ROUND_DOWN;
	data[0] = vor0_val[0] + 
							taylor_relconst[8]*dih_val[0];
	ROUND_UP;
	data[1] = vor0_val[1] + 
							taylor_relconst[9]*dih_val[1];
	...
	if( info.depth > PARTIALSTART )	{
		...
		for( i=0; i<12; i+=2 ) {
			i_mult( taylor_relconst + 8, partials + i, t + i );
		}
		ROUND_DOWN;
		for( i=0; i<12; i+=2 ) {
...
void sean_init( void )
{
	...
	t0_val[0] = 0.5*TWOSQRT2_LO;
	t0_val[1] = 0.5*TWOSQRT2_HI;
	ROUND_DOWN;
