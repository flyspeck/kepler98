...
void verifyocta( void )
{
	...
	printf("Welcome to Section A.4.6.8.  \n");
	...
	for( n=2; n < 3; n++ ) { 
		...
		if( n==1 ) {
			i_recognize( -0.168, tetdiff );
		} else {
			i_recognize( -0.31, tetdiff );
...
void verify_cell( double y[12], struct Cell_Info info )
{
	...
	double data[2], partials[12], rel_part[12];
	...
	i_vor0( y, x, sqrtdelta, vor01_val );
	...
	if( info.depth > PARTIALSTART )	{
		...
		ROUND_DOWN;
		rel_part[0] = t[0] + partials[0];
		rel_part[2] = t[2];
		rel_part[4] = t[4] + partials[4];
		rel_part[6] = t[6];
		rel_part[8] = t[8] + partials[8];
		rel_part[10] = t[10];
		ROUND_UP;
		rel_part[1] = t[1] + partials[1];
		rel_part[3] = t[3];
		rel_part[5] = t[5] + partials[5];
		rel_part[7] = t[7];
		rel_part[9] = t[9] + partials[9];
		rel_part[11] = t[11];
