...
int hasPoints(double ymin[13],double ymax[13],int tx[4])
	...
	// bad bugged fixed in the next four lines on Dec 17, 1997.
	...
	// if one face is small, the other large, check both constraints
	// can be made to hold simultaneously.
	if (tx[0]&&!tx[1]&&(edge(ymax[1],ymax[5])>edge(ymin[2],ymin[4])))
			return 0;
	if (tx[1]&&!tx[2]&&(edge(ymax[2],ymax[4])>edge(ymin[7],ymin[8])))
			return 0;
	if (tx[2]&&!tx[3]&&(edge(ymax[7],ymax[8])>edge(ymin[10],ymin[11])))
			return 0;
	if (tx[3]&&!tx[0]&&(edge(ymax[10],ymax[11])>edge(ymin[1],ymin[5])))
			return 0;
	if (tx[0]&&!tx[3]&&(edge(ymax[1],ymax[5])>edge(ymin[10],ymin[11])))
			return 0;
	if (tx[3]&&!tx[2]&&(edge(ymax[10],ymax[11])>edge(ymin[7],ymin[8])))
			return 0;
	if (tx[2]&&!tx[1]&&(edge(ymax[7],ymax[8])>edge(ymin[2],ymin[4])))
			return 0;
	if (tx[1]&&!tx[0]&&(edge(ymax[2],ymax[4])>edge(ymin[1],ymin[5])))
			return 0;
	return 1;
	}
